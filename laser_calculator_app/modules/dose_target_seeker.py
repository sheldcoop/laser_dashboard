...
def render():
        ...
        try:
            ...
            if solve_for == "Required Average Power":
                if number_of_shots <= 0:
                    st.error("Number of Shots must be greater than zero."); return
                ...
                if avg_power_mW <= 0:
                    st.error("Average Power must be greater than zero."); return
                pulse_energy_uJ = avg_power_mW / rep_rate_khz
                pulse_energy_J = pulse_energy_uJ * UJ_TO_J
                peak_fluence = (2 * pulse_energy_J) / area_cm2
                if peak_fluence <= 0:
                    st.error("Calculated Peak Fluence is zero. Cannot achieve target dose."); return
    ...
    if 'dose_results' in st.session_state:
        ...
        res_col1.metric(keys[0], results[keys[0]])
        res_col2.metric(keys[1], results[keys[1]])
        res_col3.metric(keys[2], results[keys[2]])
